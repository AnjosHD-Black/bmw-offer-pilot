...
def parse_additions_from_priced_lines(lines: List[str]) -> List[Dict]:
    """
    Extrahiert Additions (Zeilen die mit ADD beginnen) aus priced_lines.
    Format: 'ADD1 Extended Warranty 899.00'
    Gibt Liste von {code, text, price} zurück.
    """
    additions = []
    addition_regex = re.compile(
        r"^(?P<code>ADD\d+)\s+(?P<text>.+?)\s+(?P<price>\d[\d.,]*\d|\d)$",
        re.IGNORECASE
    )
    ...
    for line in lines:
        ...
        match = addition_regex.match(line)
        if match:
            additions.append({
